...
dataDIR="../Datasets/MERRA-PreMon.nc"
...
plt.pcolor(lon, lat, AOD_mean,
            transform=ccrs.PlateCarree(),cmap='jet',vmin=.20,vmax=.8)
plt.colorbar()    
...
plt.savefig('MERRA_AOD_MEAN-PreMon(2005-2021).png',dpi=300)
# plt.show()
plt.clf()
...
dataDIR="../Datasets/MERRA-Winter.nc"
data = xr.open_dataset(dataDIR)
AOD=data.AOD
AOD=AOD[:,:,:].astype(np.double)
...
plt.savefig('MERRA-AOD_MEAN-WINTER(2005-2021).png',dpi=300)
# plt.show()